# Providers Table Setup Guide

## Issue
The `providers` table does not exist in the Supabase database, causing 404 errors when the application tries to fetch vetted service providers.

## Solution
Run the following SQL in your Supabase SQL Editor to create the `providers` table and seed it with initial data.
...
## Step 3: Seed the Providers Data

Run this SQL to add initial provider data:

```sql
-- Seed with compliance service providers
INSERT INTO providers (name, logo_url, description, categories, tier, website_url, website, contact_email, contact_phone, services, states_covered, pricing_tier, rating, review_count, verified, featured, category)
VALUES
  (
    'ComplianceForge',
...
The table already exists. You can skip Step 2 and just run the seed data in Step 3.

### Error: "duplicate key value violates unique constraint"
The seed data already exists. The `ON CONFLICT` clause should handle this, but if you see this error, you can run:
```sql
...
Make sure RLS is enabled and the policies are created correctly. Re-run the policy creation statements from Step 2.

## Related Files
- `supabase/migrations/create_providers_table.sql` - Full migration file
- `src/components/ProviderCard.tsx` - Frontend component that displays providers
- `src/components/AppLayout.tsx` - Main layout that fetches providers

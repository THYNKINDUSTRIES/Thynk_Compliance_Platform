...
export const NicotineVapePoller: React.FC = () => {
  ...
  const fetchLogs = async () => {
    try {
      const { data, error } = await supabase
        .from('ingestion_log')
        .select('*')
        .eq('source', 'nicotine-vape-poller')
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({ totalDocuments: documents.length, lastUpdated, documentTypes });
    } catch (error) {
  ...
  const runPoller = async () => {
    ...
    try {
      const response = await supabase.functions.invoke('nicotine-vape-poller', {
      ...
      if (data.success) {
        toast({
          title: "Nicotine/Vape Poller Completed",
          description: `Processed ${data.processedItems || 0} regulatory items`,
        });
        await Promise.all([fetchLogs(), fetchRecentDocuments(), fetchStats()]);
      } else {
      ...
      toast({
        title: "Poller Not Deployed",
        description: 'The nicotine/vape poller edge function has not been deployed yet. Deploy it via Supabase CLI.',
        variant: "destructive",
      });
    } finally {
      setIsPolling(false);
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Nicotine &amp; Vape Regulatory Poller
        </CardTitle>
        <CardDescription>
          Monitor nicotine, e-cigarette, vaporizer, and tobacco product regulations across federal and state agencies
        </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        ...
        <div className="flex gap-2 mb-6">
          <Button onClick={runPoller} disabled={isPolling} className="flex items-center gap-2">
            {isPolling ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
            {isPolling ? 'Running...' : 'Run Nicotine/Vape Poller'}
          </Button>
          <Button variant="outline" onClick={() => { fetchLogs(); fetchRecentDocuments(); fetchStats(); }} className="flex items-center gap-2">
        ...
        {isPolling && (
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground mt-2">Processing nicotine &amp; vape regulations...</p>
          </div>
        )}
            ...
            <ScrollArea className="h-96">
              {recentDocuments.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  No nicotine/vape documents found. Run the poller to fetch regulations.
                </div>
              ) : (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <Badge variant="secondary" className={docType.color}>{docType.label}</Badge>
                            ...
                            <div className="flex items-center gap-4 text-xs text-muted-foreground">
                              ...
                              {doc.url && (
                                <a href={doc.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-blue-600">
                  ...
                  <Card key={log.id} className="mb-3">
                    <CardContent className="pt-4">
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                        <div><span className="text-muted-foreground">Fetched:</span> {log.records_fetched}</div>
                        <div><span className="text-muted-foreground">Created:</span> {log.records_created}</div>
                      </div>
                      {log.error_message && (
                        <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-800">{log.error_message}</div>
                      )}
                    </CardContent>
                  </Card>
                ))

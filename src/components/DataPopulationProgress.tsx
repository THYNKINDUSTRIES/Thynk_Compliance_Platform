...
export function DataPopulationProgress({ sessionId, onComplete }: Props) {
  ...
  const totalFetched = progress.reduce((sum, p) => sum + p.records_fetched, 0);
  const totalEstimated = progress.reduce((sum, p) => sum + p.total_estimated, 0);
    ...
    <Card>
      <CardHeader>
        ...
        <div className="text-sm text-muted-foreground">
          {totalFetched} / {totalEstimated} regulations fetched
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        {progress.map((record) => (
          <div key={record.id} className="space-y-2">
            ...
            <Progress value={(record.records_fetched / record.total_estimated) * 100} />
          </div>
        ))}
      </CardContent>
    </Card>

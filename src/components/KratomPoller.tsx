...
export const KratomPoller: React.FC = () => {
  ...
  const runPoller = async () => {
    ...
    try {
      const response = await supabase.functions.invoke('kratom-poller', {
        body: {
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Kratom Regulatory Poller
        </CardTitle>
        <CardDescription>
          Monitor federal kratom regulations from RSS feeds
        </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        ...
        <div className="flex gap-2 mb-6">
          ...
          >
            ...
            {isPolling ? 'Running...' : 'Run Kratom Poller'}
          </Button>
            ...
            <ScrollArea className="h-96">
              {recentDocuments.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  No kratom documents found. Run the poller to fetch regulations.
                </div>
              ) : (

# Checklist Templates Database Setup

## Overview

This guide explains how to:
1. Set up the `checklist_templates` table with proper columns
2. Seed the table with 8 default compliance templates
3. Update the `generate-checklist-from-template` edge function
...
```sql
-- Ensure the checklist_templates table has all required columns
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'checklist_templates' AND column_name = 'category') THEN
    ALTER TABLE checklist_templates ADD COLUMN category TEXT DEFAULT 'general';
  END IF;
  
  -- Add states column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'checklist_templates' AND column_name = 'states') THEN
    ALTER TABLE checklist_templates ADD COLUMN states TEXT[] DEFAULT '{}';
  END IF;
END $$;
...
Run this query to verify the templates were created:
...
Expected output:
| name | business_type | category | usage_count | average_rating | item_count |
|------|---------------|----------|-------------|----------------|------------|
| Cannabis Cultivator Startup Checklist | cultivator | new_license | 245 | 4.8 | 12 |
| Dispensary Annual Renewal | retailer | annual_renewal | 189 | 4.6 | 10 |
| Manufacturing Facility Inspection Prep | manufacturer | inspection_prep | 156 | 4.9 | 11 |
| New Product Launch Compliance | all | product_launch | 134 | 4.5 | 10 |
| Hemp CBD Product Compliance | manufacturer | product_launch | 112 | 4.4 | 9 |
| Delivery Service Compliance | retailer | new_license | 98 | 4.6 | 10 |
| Testing Lab Accreditation | testing_lab | new_license | 87 | 4.7 | 10 |
| Social Equity Application Support | all | new_license | 76 | 4.8 | 10 |

## Step 3: Update Edge Function

See `GENERATE_CHECKLIST_FIX.md` for the updated edge function code that:
- Fixes `.single()` 406 errors by using `.limit(1)`
- Adds AI enhancement with OpenAI
...
|----------|-------------|
| `new_license` | For new license applications |
| `annual_renewal` | For annual license renewals |
| `inspection_prep` | For preparing for inspections |
| `product_launch` | For launching new products |
| `general` | General compliance checklists |

## Business Types

| Type | Description |
|------|-------------|
| `cultivator` | Cannabis cultivation facilities |
| `retailer` | Dispensaries and retail stores |
| `manufacturer` | Manufacturing and processing |
| `testing_lab` | Testing laboratories |
| `all` | Applicable to all business types |

## Using Templates in the App

1. Navigate to **Template Library** in the app
2. Browse templates by category or business type
3. Click **Use Template** to create a checklist
4. Optionally enable AI enhancement for additional items and tips
5. The checklist is created with all template items plus AI-generated items
